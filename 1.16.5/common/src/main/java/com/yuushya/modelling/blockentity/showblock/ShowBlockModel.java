...
public class ShowBlockModel implements BakedModel, UnbakedModel {
    ...
    public List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction side, @NotNull Random rand, List<TransformData> transformDatas) {
            ...
            for (Direction value : directions) {
                ...
                for (BakedQuad bakedQuad : blockModelQuads) {
                    ...
                    if (bakedQuad.getTintIndex() > -1)//将方块状态和颜色编码到tintindex上，在渲染时解码找到对应颜色
                        ...
                        finalQuads.add(new BakedQuad(vertex, bakedQuad.getTintIndex(), bakedQuad.getDirection(), bakedQuad.sprite, bakedQuad.isShade()));
                }
            }
        }
        return finalQuads;
